...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussshejirenwuCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussshejirenwuCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussshejirenwu', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussshejirenwu');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussshejirenwu');
			...
			async init(){
				...
				let res = await this.$api.info('discussshejirenwu', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussshejirenwu', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussshejirenwu/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){

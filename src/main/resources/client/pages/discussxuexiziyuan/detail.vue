...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussxuexiziyuanCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussxuexiziyuanCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussxuexiziyuan', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussxuexiziyuan');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussxuexiziyuan');
			...
			async init(){
				...
				let res = await this.$api.info('discussxuexiziyuan', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussxuexiziyuan', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussxuexiziyuan/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
